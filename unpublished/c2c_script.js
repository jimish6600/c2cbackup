  ...
  new Vue({
    ...
    data() {
      return {
        ...
        metaFieldDropZoneOptions: {
          ...
          addRemoveLinks: true,
          dictDefaultMessage:
            'Please drag and drop images here or click to select files',
        ...
        dropzoneOptions: {
          ...
          dictRemoveFile: 'Remove',
          acceptedFiles:
            '.jpeg,.jpg,.png,.gif,.JPEG,.JPG,.PNG,.GIF,.webp, video/*',
          timeout: 120000,
          removeType: 'server',
        },
    ...
    methods: {
      ...
      setAuthToken() {
        return new Promise((resolve, reject) => {
          axios
            .get(`${this.baseUrl}/v2/shops/check-existence`, {
              params: {
                shop_domain: this.shop_domain,
                login_customerId: ShopifyAnalytics.meta.page.customerId,
                customer_email: this.shopifyLoggedInCustomerEmail,
              },
            })
            .then((res) => {
      ...
      setupUserDataAndPermissions() {
        return new Promise((resolve, reject) => {
          ...
          this.$nextTick(() => {
            ...
            axios.get(myVisibilitiesEndPoint, { headers }).then((res) => {
              ...
              if (typeof permissions === 'object') {
                this.allPermissions = Object.keys(permissions).map(
                  (key) => permissions[key],
                );
              } else {
      ...
      metaFieldPrepopulateFile(item) {
        let value =
          this.metaData[item.name_slug] && this.metaData[item.name_slug].value;
        if (value && item.belongs_to === 'channel') {
          ...
          this.$refs['metafieldDropzone_' + item.field_name][0].manuallyAddFile(
            file,
            file.src,
          );
        }
      ...
      prePopulateFIle() {
        ...
        this.display_videos &&
          this.productData.display_videos.forEach((video) => {
      ...
      errorEvent(files, message, xhr) {
        ...
        this.showToast(
          `Sorry the file was not successfully imported ${message}`,
          'error',
        );
      ...
      metaFieldSuccessEvent(file, response) {
        ...
        files.push({
          ...
          src: response.data.file_link_url
            ? response.data.file_link_url[0]
            : '',
          size: file.size,
          name: file.name,
          type: file.type,
      ...
      imageUploadInProgress(isUploading) {
        if (isUploading) {
          const wizardFooterRight = document.getElementsByClassName(
            'wizard-footer-right',
          );
          wizardFooterRight[0].getElementsByClassName(
            'wizard-btn',
          )[0].disabled = true;
        } else {
          const wizardFooterRight = document.getElementsByClassName(
      ...
      registerVendor() {
        ...
        axios
          .post(`${this.baseUrl}/v2/create-new-vendor-v2`, formData)
          .then((response) => {
          ...
          .catch((error) => {
            ...
            this.showToast(
              'Error while registrating, please try again later.',
              'failure',
            );
          })
          .finally(() => {
      ...
      async fetchCountries() {
        try {
          ...
          const { data } = await axios.get(`${this.baseUrl}/v1/country`, {
            headers,
          });
          this.countries = data;
        } catch (error) {
      ...
      async fetchProducts() {
        try {
          ...
          const response = await axios.get(
            `${this.baseUrl}/v1/customer-to-customer/product/parent?limit=10&allproducts=true`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('stAuth')}`,
              },
      ...
      async fetchCategories() {
        try {
          ...
          const response = await axios
            .get(
              `${this.baseUrl}/v1/product-type/product-type-autocomplete/ + ${encodeURI(queryparams)}`,
              {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem('stAuth')}`,
                },
      ...
      async fetchActiveProducts(type = null) {
        try {
          ...
          const activeProductsResponse = await axios.get(
            `${this.baseUrl}/v1/customer-to-customer/product/parent?query=%7B%22status%22:%22active%22%7D&limit=${this.itemsPerPage}&ascending=0&page=${this.pageNumber}&byColumn=1
                        `,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('stAuth')}`,
      ...
      async fetchPendingApprovalProducts() {
        ...
        await axios
          .get(
            `${this.baseUrl}/v1/customer-to-customer/draft-product?query=%7B%7D&limit=100&ascending=0&page=1&byColumn=1
                    `,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('stAuth')}`,
          ...
          .catch((error) => {
            ...
            this.showToast(
              'Error while fetching pending approval products',
              'failure',
            );
          })
          .finally(() => {
      ...
      getProductCount() {
        axios
          .get(`${this.baseUrl}/v1/products/count-of-draft-product/all`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('stAuth')}`,
            },
          })
      ...
      async setCustomField() {
        ...
        this.customFields = customFieldResponse.data.map((customField) => {
          if (
            customField.attributes.type === 'dimension' &&
            customField.value
          ) {
      ...
      setProductData(data) {
        ...
        this.productData.quantity = this.firstVariant
          ? this.firstVariant.inventory_quantity
          : data.inventory_quantity;
        this.productData.price = this.firstVariant
          ? this.firstVariant.price
          : data.price;
        this.productData['length'] = this.firstVariant
          ? this.firstVariant.length
          : data['length'];
        this.productData.width = this.firstVariant
          ? this.firstVariant.width
          : data.width;
        this.productData.height = this.firstVariant
          ? this.firstVariant.height
          : data.height;
      ...
      popoverMethodForCommission(productTitle, commission, currency, earnings) {
        return (
          '<div class="p-4 st-commission-container"><h4 class="mb-4 bold">' +
          productTitle +
          '</h4><b class="mb-2">Commission: ' +
          commission +
          '%</b> </br> <b class="mb-4">Your earnings: ' +
          currency +
          earnings +
          '</b><p class="mb-4">For each product you sell, commissions are set by the merchant. This can be subjective to various factors including the product category or other info you provide </p> </div>'
        );
      },
      ...
      async addProduct() {
        try {
          ...
          formData.append(
            'merchant_comments',
            this.productData.merchant_comments,
          );
          formData.append('requires_shipping', this.requiresShippingValue);
          formData.append(
            'rental_rule_id',
            this.productData.rental_rule_id || '',
          );
      ...
      async editProduct() {
        try {
          ...
          formData.append(
            'admin_graphql_api_id',
            this.productData.admin_graphql_api_id,
          );
          formData.append(
            'default_variant_id',
            this.firstVariant
              ? this.firstVariant.id
          ...
          formData.append(
            'old_images',
            JSON.stringify(this.productData.old_images),
          );
          formData.append(
            'old_videos',
            JSON.stringify(this.productData.old_videos),
          );
          formData.append('images', JSON.stringify(this.images));
          formData.append('videos', JSON.stringify(this.productData.videos));
          ...
          if (this.firstVariant) {
            ...
            formData.append(
              'options',
              JSON.stringify(this.productData.options),
            );
            formData.append(
              'options[0]',
              JSON.stringify(this.productData.options[0]),
            );
          }
      ...
      async openEditProduct(productData, productType, pageName) {
        ...
        try {
          ...
          if (productType === 'pendingApproval') {
            const response = await axios.get(
              `${this.baseUrl}/v1/products/fetch-draft-product/${this.editProductId}`,
              { headers },
            );
            let data = {
              ...response.data.data.product_json,
              merchant_comments: response.data.data.merchant_comments,
            };
            this.setProductData(data);
            await this.setCustomField();
          } else {
            const productResponse = await axios.get(
              `${this.baseUrl}/v1/products/fetch-product/${this.editProductId}`,
              { headers },
            );
            let data = {
              ...productResponse.data.product_json,
              merchant_comments: productResponse.data.merchant_comments,
            };
            this.setProductData(data);
            await this.setCustomField();
          }
      ...
      validateCustomFields() {
        for (const customField of this.userSelectedCustomFields) {
          ...
          if (!value) {
            this.addEditProductErrors[customField.name_slug] =
              `${customField.field_name} is required`;
          }
        }
      },
      ...
      async fetchSingleOrder(id) {
        try {
          this.loading = true;
          const response = await axios.get(
            `${this.baseUrl}/v1/orders/line-items/${id}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('stAuth')}`,
              },
      ...
      async rejectBarter() {
        try {
          ...
          const response = await axios.post(
            `${this.baseUrl}/v1/barter-draft-orders/rejectBarterDraftOrder`,
            payload,
            { headers },
          );
      ...
      async fetchBarters(type) {
        ...
        try {
          ...
          const headers = this.getAuthHeaders();
          const response = await axios.post(
            `${this.baseUrl}/v3/barter-draft-orders/fetch`,
            {},
            { headers },
          );
          const { data } = response.data;
          this.barters =
            type === 'infinite-scrolling' ? [...this.barters, ...data] : data;
      ...
      async approveBarter() {
        try {
          ...
          const headers = this.getAuthHeaders();
          const response = await axios.post(
            this.baseUrl + '/v1/barter-draft-orders/acceptBarterDraftOrder',
            { draft_order_id: this.singleBarterData.draft_order_id },
            { headers },
          );
          this.showToast(
            response.data.message || 'Barter accepted successfully',
            'success',
          );
      ...
      getCounterOfferOrderValue(data) {
        return (
          this.getCounterOfferPrice(data) * this.getCounterOfferQuantity(data)
        );
      },
      ...
      calculateOfferPricePercentage(data) {
        try {
          const percentage =
            (this.getOfferPrice(data) / Number(data.line_items[0].price)) *
              100 -
            100;
      ...
      async counterOfferSubmit(payload) {
        try {
          ...
          const headers = this.getAuthHeaders();
          const response = await axios.post(
            this.baseUrl + '/v1/draft-orders/counterOfferDetails',
            payload,
            { headers },
          );
          if (response.status === 200) {
      ...
      async approve() {
        try {
          ...
          const headers = this.getAuthHeaders();
          const response = await axios.post(
            this.baseUrl + '/v1/draft-orders/acceptDraftOrder',
            { draft_order_id: this.singleOfferData.draft_order_id },
            { headers },
          );
          if (response.status === 200) {
      ...
      async reject() {
        try {
          ...
          const headers = this.getAuthHeaders();
          const response = await axios.post(
            this.baseUrl + '/v1/draft-orders/rejectDraftOrder',
            {
              draft_order_id: this.singleOfferData.draft_order_id,
      ...
      async fetchOffers(type) {
        ...
        try {
          ...
          this.offers =
            type === 'infinite-scrolling' ? [...this.offers, ...data] : data;
      ...
      async fetchChats(type) {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            this.chats =
              type === 'infinite-scrolling' ? [...this.chats, ...data] : data;
      ...
      async deleteMessage(chatData) {
        ...
        try {
          this.chats_loading = true;
          const response = await axios.post(chatsEndpoint, payload, {
            headers,
          });
          this.fetchChats();
        } catch (error) {
      ...
      getPayloadForChatStatus(chatData) {
        ...
        return newStatus
          ? { query: { id: chatData.id, status: newStatus } }
          : null;
      },
      ...
      async storeProductLevelChatReply(payload) {
        ...
        const params = this.createFormData(payload, [
          'message',
          'shopify_domain',
          'product_id',
          'customer_email',
      ...
      async storeMessageData(value) {
        ...
        const params = this.createFormData(value, [
          'message',
          'company_query_id',
          'is_master',
        ]);
      ...
      updateOfferPickUpFromDefaultWarehouse(value) {
        ...
        axios
          .post(`${this.baseUrl}/v1/pickup_option_at_checkout`, params, {
            headers,
          })
          .then((response) => {
            if (value) {
      ...
      async editProfile() {
        ...
        try {
          const response = await axios.get(
            `${this.baseUrl}/v1/customer-to-customer/company`,
            { headers },
          );
      ...
      savePaymentMethod() {
        ...
        if (this.paymentMethod.id) {
          axios
            .put(
              `${this.baseUrl}/v1/payment-gateways/${this.paymentMethod.id}`,
              params,
              { headers },
            )
            .then((res) => {
          ...
          axios
            .post(`${this.baseUrl}/v1/payment-gateways`, params, { headers })
            .then((res) => {})
            .finally(() => {
              this.submitLoading = false;
            });
      ...
      async connectVendorAccount(id) {
        try {
          const headers = this.getAuthHeaders();
          const response = await axios.get(
            `${this.baseUrl}/v1/payment-gateways/connect-vendor-account/${id}?marketplace=c2c&domain=${window.location.host}`,
            { headers },
          );
      ...
      async getPaymentMethodDetails() {
        ...
        try {
          ...
          const response = await axios.get(
            `${this.baseUrl}/v1/payment-gateways`,
            { headers },
          );
          const [data] = response.data.data;
      ...
      convert24HourFormatBookingType() {
        if (this.formBookingType.available_date_range.start_time) {
          this.formBookingType.available_date_range.start_time =
            this.to24HourFormat(
              this.formBookingType.available_date_range.start_time,
            );
        }
        if (this.formBookingType.available_date_range.start_time) {
          this.formBookingType.available_date_range.end_time =
            this.to24HourFormat(
              this.formBookingType.available_date_range.end_time,
            );
        }

        for (const [day, schedule] of Object.entries(
          this.formBookingType.operating_hours,
        )) {
      ...
      async openEditBookingType(bookingTypeId) {
        ...
        const headers = this.getAuthHeaders();
        try {
          const response = await axios.get(EditBookingTypeEndpoint, {
            headers,
          });
      ...
      async fetchBooking() {
        ...
        try {
          ...
          this.totalPagesBookingType = Math.ceil(
            response.data.count / this.perPageBookingType,
          );
        } catch (error) {
          console.error('Error fetching auction products:', error);
        } finally {
      ...
      async inactiveBookingType(bookingPayload) {
        ...
        const headers = this.getAuthHeaders();
        try {
          await axios.post(inactiveBookingTypeEndpoint, bookingPayload, {
            headers,
          });
          this.fetchBooking();
          ...
          this.showToast(
            'Failed to inactivate booking rule. Please try again.',
            'error',
          );
        } finally {
        }
      ...
      async activeBookingType(bookingPayload) {
        ...
        const headers = this.getAuthHeaders();
        try {
          await axios.post(activeBookingTypeEndpoint, bookingPayload, {
            headers,
          });
          this.fetchBooking();
          ...
          this.showToast(
            'Failed to activate booking rule. Please try again.',
            'error',
          );
        } finally {
        }
      ...
      async updateBookingType() {
        if (
          !this.validateCutoffDaysBookingType ||
          !this.validateFutureDaysBookingType
        ) {
        ...
        try {
          await axios.put(saveBookingTypeEndpoint, this.formBookingType, {
            headers,
          });
      ...
      async saveBookingType() {
        ...
        try {
          await axios.post(saveBookingTypeEndpoint, this.formBookingType, {
            headers,
          });
          this.openPage('Booking');
          this.fetchBooking();
    ...
    watch: {
      ...
      counterOfferQuantity: {
        handler(newVal) {
          this.counterOfferQuantityChanged =
            newVal !== this.singleOfferData.line_items[0].quantity;
        },
        immediate: false,
        deep: true,
      },
      counterOfferPrice(newVal) {
        this.counterOfferPriceChanged =
          newVal !== this.getOfferPrice(this.singleOfferData);
      },
    },
    computed: {
      currencySymbol() {
        return (
          this.userData?.default_shop?.currency_country?.currency_symbol ||
          this.userData?.default_shop?.currency ||
          ''
        );
      },
      /*Booking Page*/
      ...
      getModalTitle() {
        return this.pickupAction === 'addNewWarehouse'
          ? 'Add Pickup Address'
          : 'Edit Pickup Address';
      },
      isMobile() {
      ...
      totalActiveProductsPages() {
        ...
        const totalPages = Math.ceil(
          this.activeProductsCount / this.itemsPerPage,
        );
        return totalPages;
      },
      totalChatsPages() {
      ...
      storeSellsOnlyDigitalProduct() {
        return (
          this.userData.default_shop.product_types_sold_on_store ===
          'digital_products_only'
        );
      },
      ...
      formattedDate() {
        if (this.singleChatData.created_at) {
          return new Date(this.singleChatData.created_at)
